...
async function loadChapterContent(bookId, chapterId) {
  ...
  try {
    ...
    if (!allChapters || allChapters.length === 0) {
      // Display a friendly message when no chapters are found
    ...
    if (!chapter || !chapter.content) {
      // Display a friendly message when the specific chapter doesn't exist or has no content
    ...
    // Add author information (if the author element exists)
    ...
    // Display a friendly error message instead of technical error
