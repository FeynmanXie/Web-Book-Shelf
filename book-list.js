...
async function loadBooks() {
  ...
  try {
    ...
    // Get all chapters (with covers), sorted by book_id + chapter_id
    ...
    if (!Array.isArray(chapters) || chapters.length === 0) {
      // Create a "No Books Here Yet..." message instead of an error
    ...
    // Extract the first chapter of each book
  ...
  card.innerHTML = `
    <a href="book.html?book=${encodeURIComponent(bookId)}" class="block">
      ...
      <div class="p-4">
        ...
        <p class="text-amber-700 dark:text-amber-300 mt-1">Author: <span class="font-semibold">FeynmanXie</span></p>
        <p class="text-amber-700 dark:text-amber-300 mt-2">Click to view chapters</p>
      </div>
    </a>
  `;
