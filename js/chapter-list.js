...
async function loadChapters(bookId) {
  ...
  try {
    ...
    console.log('📘 加载章节数据：', chapters); // ✅ 调试关键
    ...
    if (chapters[0] && chapters[0].created_at) {
      ...
      if (createdDateElement) {
        createdDateElement.textContent = `创建于：${formattedDate}`;
      }
    }
...
function createChapterItem(chapter, index) {
  ...
  if (!chapterId || !bookId) {
    console.warn('⛔️ 缺失 chapterId 或 bookId：', chapter);
    return document.createElement('div'); // 返回空元素，避免报错
  }
